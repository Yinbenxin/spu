...
# def old_psi(role, inputs, element_size):
    
#     psi_party = psi.pypsi_party(log_dir='./', log=True, debug=False,
#                                 check_path="/cube/bin/check.x", license_path="/data/gaia_license/license.dat")

#         # 不同角色共用的一些参数。   nelements：用于dbg
#     party, redis = get_server_from_env() 
#     psi_params = dict(nelements=0, elementbylen=element_size, address=party,
#                     client_address=redis,
#                     server_output=True, use_redis=True, read_from_file=False, type=3)
#     taskid = "psitask"
#     if role == 1:
#         param = psi.PSIParameters(taskid, 1, inputs, outfilename=None, **psi_params)
#         ids_idx = psi_party.batch_psi_client(param)
#     else:
#         param = psi.PSIParameters(taskid, 0, inputs, outfilename=None, **psi_params)
#         ids_idx = psi_party.batch_psi_server(param)
#     return ids_idx
...
def pressure_test():
    ...
    if args.role == -1:
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=2) as executor:
            futures.append(executor.submit(psi_party, ROLE_SERVER, recv_list_bytes, element_size))
            futures.append(executor.submit(psi_party, ROLE_CLIENT, sender_list_bytes, element_size))
        for f in futures:
            res_idx = f.result()
            print("res len {}, expect len {}", len(res_idx), intersection_size)
        res_idx = futures[1].result()
        assert len(res_idx) == intersection_size
    elif args.role == 0:
